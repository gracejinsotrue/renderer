...
6) CUDA-accelerated triangle rasterization (batched). CPU rasterizer does one triangle at a time, one core does all the work so true. Basically the CUDA path queues all triangles into a staging buffer and uploads them to the GPU in one shot. Each thread is one pixel, loops over the triangle batch, and uses precomputed bounding boxes to skip triangles that don't cover it. Offscreen triangles get culled before they even hit the GPU. Tradeoff is still memory transfer overhead since the frame has to come back to CPU, but the single bulk upload means it wins on models above ~1-2k triangles, so basically non-low poly models
5)  Möller-Trumbore Ray-Triangle Intersection technique for ray tracing path, it's a classic fast algorithm for ray-mesh intersection testing. [more about Möller-Trumbore](https://en.wikipedia.org/wiki/M%C3%B6ller%E2%80%93Trumbore_intersection_algorithm)
6) A classic Bounding Volume Hierarchy (BVH) acceleration structure for ray tracing! This builds upon Moller Trumbore and this is probably the easiest way to speed up anyone's ray tracer aside from like, backface culling stuff. This builds a spatial tree with axis-aligned bounding boxes (AABB) and. enables fast ray-object intersection testing in O(log n) time instead of O(n), drastically improving real-time ray tracing performance. [more about BVH accelerated ray tracing](https://en.wikipedia.org/wiki/Bounding_volume_hierarchy)
