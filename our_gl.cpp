// our_gl.cpp--where our rendering functions are
#include <cmath>
#include <limits>
#include <cstdlib>
#include "our_gl.h"
...
// global shadow buffer - will be resized in main
...
Vec3f barycentric(Vec2f A, Vec2f B, Vec2f C, Vec2f P)
{
    ...
    for (int i = 2; i--;)
    {
        s[i][0] = C[i] - A[i]; // vector ac
        s[i][1] = B[i] - A[i]; // vector ab
        s[i][2] = A[i] - P[i]; // vector ap
    }
    Vec3f u = cross(s[0], s[1]);

    if (std::abs(u[2]) > 1e-2)                                       // dont forget that u[2] is integer. If it is zero then triangle ABC is degenerate
        return Vec3f(1.f - (u.x + u.y) / u.z, u.y / u.z, u.x / u.z); // rturn the noramlized barycentric coord, they must sum up to 1
    return Vec3f(-1, 1, 1);                                          // return invalid cord for degenrate triangles                                      // in this case generate negative coordinates, it will be thrown away by the rasterizator
}
