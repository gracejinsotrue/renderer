...
struct vec<2, T>
{
    ...
    vec(T X, T Y) : x(X), y(Y) {}
    template <class U>
    vec<2, T>(const vec<2, U> &v) : x(T(v.x)), y(T(v.y)) {} // Implementation added
...
struct vec<3, T>
{
    ...
    template <class U>
    vec<3, T>(const vec<3, U> &v) : x(T(v.x)), y(T(v.y)), z(T(v.z)) {} // Implementation added
...
public:
    ...
    // matrix inverse using adjugate method lol

    mat<DimRows, DimCols, T> invert_transpose()
    {
        // for square marixes inverse = adjugate/determinant
